 ...
 *
 * Reactor is VIDEO ONLY: FastH3 films the beats, and nothing else on that key is
 * called. Stills are nano-banana again — but on Google's own platform rather
 * than through a video provider. They used to be invoked from separate routes that
...
/** Vertex commonly puts google.rpc.RetryInfo in the JSON body, not a header. */
function retryAfterBodyMs(detail: string): number | undefined {
  const match = /"retryDelay"\s*:\s*"([0-9]+(?:\.[0-9]+)?)s"/.exec(detail);
  if (!match) return undefined;
  const delay = Number(match[1]) * 1_000;
  return Number.isFinite(delay) ? Math.max(0, delay) : undefined;
}
...
async function googleImage(request: ImageRequest): Promise<Buffer> {
  ...
  if (!response.ok) {
    ...
    const headerWait = retryAfterMs(response.headers.get("retry-after")) ?? 0;
    const bodyWait = retryAfterBodyMs(detail) ?? 0;
    const hint =
    ...
    throw new ImageGenError(
      ...
      response.status,
      Math.max(headerWait, bodyWait) || undefined
    );
  }
  ...
  throw new ImageGenError(
    "No image provider is configured. Set IMAGE_PROVIDER in .env.local " +
      `(available: ${Object.keys(PROVIDERS)
        ...
        .join(", ")}), or add one to lib/imagegen.ts. Reactor is video-only here.`
  );
}
 ...
 * which names what to do. It never falls back to Reactor — that is the whole
 * point of this file.
 */
...
// Image generation is a scarce shared-capacity operation. Serialize it at
// the process boundary so a portrait, opening and story beat cannot create a
// burst even if their HTTP routes arrive together.
...
/** Convenience for callers that need generated bytes directly. */
